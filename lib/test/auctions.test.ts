...
import MOCK_USDC_ARTIFACT from "../MockUSDC.json";
...
export const auctionSystemTests = function () {
    ...
    let user3Wallet: Wallet;
    let user4Wallet: Wallet;
    ...
    before(async function () {
        ...
        user3Wallet = new Wallet(USER3_PRIVATE_KEY, provider);
        user4Wallet = new Wallet(USER4_PRIVATE_KEY, provider);
        ...
        // Get MockUSDC address from the deployed auction system
        const usdcAddress = await relayerClient.auctions.contract.coinContract();
        mockUSDC = new Contract(usdcAddress, MOCK_USDC_ARTIFACT.abi, relayerWallet);
    });
    ...
    describe("Auction Creation", function () {
        ...
        it("RELAYER should mint MockUSDC for all users", async function () {
            this.timeout(30_000);

            const tx1 = await mockUSDC.mint(await user1Client.getAddress(), 100_000_000n);
            await tx1.wait();
            expect(await mockUSDC.balanceOf(await user1Client.getAddress())).to.equal(100_000_000n);

            const tx2 = await mockUSDC.mint(await user2Client.getAddress(), 200_000_000n);
            await tx2.wait();
            expect(await mockUSDC.balanceOf(await user2Client.getAddress())).to.equal(200_000_000n);

            const tx3 = await mockUSDC.mint(await user3Client.getAddress(), 700_000_000n);
            await tx3.wait();
            expect(await mockUSDC.balanceOf(await user3Client.getAddress())).to.equal(700_000_000n);
        });
        ...
        it("USER1 should not create an auction for more than 7 days", async function () {
            ...
            const createAuctionRequest = await user1Client.auctions.createAuctionRequest(tokenId, 10_000_000n, BigInt(auctionEndTime));
        ...
        it("USER1 should create an auction", async function () {
            ...
            const {request: auctionRequest, signature: auctionSignature} = await user1Client.auctions.createAuctionRequest(tokenId, 10_000_000n, BigInt(auctionEndTime));
            ...
            expect(auction.startPrice).to.equal(10_000_000n);
    ...
    describe("Auction Bidding", function () {
        it("USER2 should not bid less than the start price", async function () {
            ...
            const approveTx = await mockUSDC.connect(user2Wallet).approve(contractAddress, 9_000_000n);
            await approveTx.wait(1);

            const bidRequest = await user2Client.auctions.createBidRequest(auctionId, 9_000_000n);
        ...
        it("USER2 should bid on the auction and RELAYER should relay the bid", async function () {
            ...
            const approveTx = await mockUSDC.connect(user2Wallet).approve(contractAddress, 50_000_000n);
            await approveTx.wait(1);

            const {request: bidRequest, signature: bidSignature} = await user2Client.auctions.createBidRequest(auctionId, 50_000_000n);
            ...
            expect(updatedAuction.highestBid).to.equal(50_000_000n);
        });

        it("USER3 should not be able to bid on the auction because he has not approved the contract", async function () {
            this.timeout(5_000);

            const {request: bidRequest, signature: bidSignature} = await user3Client.auctions.createBidRequest(auctionId, 100_000_000n);
        ...
        it("USER3 should not be able to bid less than the current highest bid", async function () {
            ...
            const approveTx = await mockUSDC.connect(user3Wallet).approve(contractAddress, 40_000_000n);
            await approveTx.wait(1);

            const {request: bidRequest, signature: bidSignature} = await user3Client.auctions.createBidRequest(auctionId, 40_000_000n);
        ...
        it("USER3 should be able to bid on the auction", async function () {
            ...
            const approveTx = await mockUSDC.connect(user3Wallet).approve(contractAddress, 100_000_000n);
            await approveTx.wait(1);
            ...
            const updatedAuction = await user1Client.auctions.getAuction(auctionId);
            expect(updatedAuction.highestBid).to.equal(100_000_000n);
        });

        it("USER2 shoud rebid on the auction", async function () {
            this.timeout(15_000);

            const approveTx = await mockUSDC.connect(user2Wallet).approve(contractAddress, 150_000_000n);
            await approveTx.wait(1);

            const {request: bidRequest, signature: bidSignature} = await user2Client.auctions.createBidRequest(auctionId, 150_000_000n);
            const bidTx = await relayerClient.auctions.relayBidRequest(bidRequest, bidSignature);
            await bidTx.wait(1);

            const updatedAuction = await user1Client.auctions.getAuction(auctionId);
            expect(updatedAuction.highestBid).to.equal(150_000_000n);
        });
        ...
        it("USER3 should not rebid on the auction because the auction has ended", async function () {
            ...
            const {request: bidRequest, signature: bidSignature} = await user3Client.auctions.createBidRequest(auctionId, 200_000_000n);
    ...
    describe("Auction Settlement", function () {
        ...
        it("USER3 should be able to withdraw his bid", async function () {
            ...
            const user3Balance = await mockUSDC.balanceOf(await user3Client.getAddress());
            expect(user3Balance).to.equal(700_000_000n);
        });
        ...
        it("USER1 should be able to withdraw the highest bid", async function () {
            ...
            const user1Balance = await mockUSDC.balanceOf(await user1Client.getAddress());
            expect(user1Balance).to.equal(250_000_000n);
        });
    ...
    describe("User Bid Tracking", function () {
        ...
        it("should prepare tokens and auctions for tracking tests", async function () {
            ...
            const tx1 = await mockUSDC.mint(await user2Client.getAddress(), 300_000_000n);
            await tx1.wait();
            const tx2 = await mockUSDC.mint(await user3Client.getAddress(), 300_000_000n);
            await tx2.wait();
            ...
            const createReq1 = await user1Client.auctions.createAuctionRequest(trackingTokenId1, 20_000_000n, BigInt(endTime1));
            ...
            const createReq2 = await user1Client.auctions.createAuctionRequest(trackingTokenId2, 30_000_000n, BigInt(endTime2));
        ...
        it("USER2 should bid on first tracking auction", async function () {
            ...
            const approveTx = await mockUSDC.connect(user2Wallet).approve(contractAddress, 25_000_000n);
            await approveTx.wait(1);

            const bidReq = await user2Client.auctions.createBidRequest(trackingAuctionId1, 25_000_000n);
        ...
        it("USER3 should bid on both tracking auctions", async function () {
            ...
            const approveTx1 = await mockUSDC.connect(user3Wallet).approve(contractAddress, 35_000_000n);
            await approveTx1.wait(1);

            const bidReq1 = await user3Client.auctions.createBidRequest(trackingAuctionId1, 35_000_000n);
            const bidTx1 = await relayerClient.auctions.relayBidRequest(bidReq1.request, bidReq1.signature);
            await bidTx1.wait(1);

            // Bid on second auction
            const approveTx2 = await mockUSDC.connect(user3Wallet).approve(contractAddress, 40_000_000n);
            await approveTx2.wait(1);

            const bidReq2 = await user3Client.auctions.createBidRequest(trackingAuctionId2, 40_000_000n);
        ...
        it("USER2 should bid again on first auction (no duplicate tracking)", async function () {
            ...
            const approveTx = await mockUSDC.connect(user2Wallet).approve(contractAddress, 45_000_000n);
            await approveTx.wait(1);

            const bidReq = await user2Client.auctions.createBidRequest(trackingAuctionId1, 45_000_000n);
            const bidTx = await relayerClient.auctions.relayBidRequest(bidReq.request, bidReq.signature);
            await bidTx.wait(1);
    ...
    describe("Events", function () {
        ...
        it("should emit OnCreate event", async function () {
            ...
            const createReq = await user1Client.auctions.createAuctionRequest(eventTokenId, 10_000_000n, BigInt(auctionEventEndTime));
        ...
        it("should emit OnBid event for USER3", async function () {
            ...
            const onBid = new Promise<void>((resolve, reject) => {
                user3Client.auctions.onBidEvent((auctionId, bidder, bidAmount) => {
                    try {
                        ...
                        expect(bidAmount).to.equal(50_000_000n);
                        resolve();
                    } catch (e) {
                        reject(e);
                    }
            ...
            const approveTx = await mockUSDC.connect(user3Wallet).approve(contractAddress, 50_000_000n);
            await approveTx.wait(1);

            const bidReq = await user3Client.auctions.createBidRequest(auctionEventId, 50_000_000n);
            const bidTx = await relayerClient.auctions.relayBidRequest(bidReq.request, bidReq.signature);
            await bidTx.wait(1);
            ...
            const onWithdraw = new Promise<void>((resolve, reject) => {
                relayerClient.auctions.onWithdrawEvent((user, amount) => {
                    try {
                        ...
                        expect(amount).to.equal(75_000_000n);
                        resolve();
                    } catch (e) {
                        reject(e);
                    }
    ...
    describe("Auction State Queries", function () {
        ...
        it("should prepare auctions for state query tests", async function () {
            ...
            const createReq1 = await user1Client.auctions.createAuctionRequest(queryTokenId1, 15_000_000n, BigInt(queryEndTime1));
            const createTx1 = await relayerClient.auctions.relayCreateAuctionRequest(createReq1.request, createReq1.signature);
            await createTx1.wait(1);
            ...
            const createReq2 = await user1Client.auctions.createAuctionRequest(queryTokenId2, 25_000_000n, BigInt(queryEndTime2));
            const createTx2 = await relayerClient.auctions.relayCreateAuctionRequest(createReq2.request, createReq2.signature);
            await createTx2.wait(1);
    ...
    describe("Auction with no bids", function () {
        it("should handle auction with no bids", async function () {
            ...
            const createAuctionRequest = await user4Client.auctions.createAuctionRequest(noBidTokenId, 1000_000_000n, BigInt(noBidAuctionEndTime));
        ...
        it("should not finalize an auction that has not ended", async function () {
            ...
            const createAuctionRequest = await user4Client.auctions.createAuctionRequest(tokenId, 10_000_000n, BigInt(auctionEndTime));
            const createTx = await relayerClient.auctions.relayCreateAuctionRequest(createAuctionRequest.request, createAuctionRequest.signature);
            await createTx.wait(1);
